    ...
    def train(self):
        ...
        for epoch in range(st_epoch + 1, self.num_epoch + 1):
            ...
            for i, data in enumerate(loader_train):
                ...
                loss_img2img_train += [loss_img2img.item()]

                print(
                    'TRAIN: EPOCH %d/%d: BATCH %04d/%04d | LR (I1I2) %.2e | LOSS (I1) %.4e | LOSS (I2) %.4e | LOSS (I1I2) %.4e'
                    % (epoch, self.num_epoch, i, num_batch_train, optim_img2img.param_groups[0]['lr'],
                       np.mean(loss_img1_train), np.mean(loss_img2_train), np.mean(loss_img2img_train)))
                ...
                if should(self.num_freq_disp):
                    ...
                    # wandb.log({'train/results_img1': example_images}, step=epoch)

            # writer_train.add_scalar('loss_prj1', np.mean(loss_prj1_train), epoch)
            writer_train.add_scalar('loss_img1', np.mean(loss_img1_train), epoch)
            writer_train.add_scalar('loss_img2', np.mean(loss_img2_train), epoch)
            writer_train.add_scalar('loss_img2img', np.mean(loss_img2img_train), epoch)

            # wandb.log({'train/loss_img1': np.mean(loss_img1_train)}, step=epoch)

            ## save
            if (epoch % self.num_freq_save) == 0:
                # self.save_single(dir_checkpoint, net_img1, optim_img1, epoch)
                self.save_dual(dir_checkpoint, net_img1, net_img2, optim_img2img, optim_img2img, epoch)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(loader_valid):
                    ...
                    print(
                        'VALID: EPOCH %d/%d: BATCH %04d/%04d | LR (I1I2) %.2e | LOSS (I1) %.4e | LOSS (I2) %.4e | LOSS (I1I2) %.4e'
                        % (epoch, self.num_epoch, i, num_batch_valid, optim_img2img.param_groups[0]['lr'],
                           np.mean(loss_img1_valid), np.mean(loss_img2_valid), np.mean(loss_img2img_valid)))
                ...
                writer_valid.add_scalar('loss_img1', np.mean(loss_img1_valid), epoch)
                writer_valid.add_scalar('loss_img2', np.mean(loss_img2_valid), epoch)
                writer_valid.add_scalar('loss_img2img', np.mean(loss_img2img_valid), epoch)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader_test):
                ...
                print(
                    'TEST: BATCH %04d/%04d | LR (I1I2) %.2e | LOSS (I1) %.4e | LOSS (I2) %.4e | LOSS (I1I2) %.4e'
                    % (i, num_batch_test, optim_img2img.param_groups[0]['lr'],
                       np.mean(loss_img1_test), np.mean(loss_img2_test), np.mean(loss_img2img_test)))
